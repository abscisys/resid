...
namespace synthaxes::hw::engine::sid
{
    ...
    // Set chip model.
    // ----------------------------------------------------------------------------
    void Voice::setChipModel(ChipModel model)
    {
        this->m_wave.setChipModel(model);
        ...
        if(model == kMos6581)
        {
            ...
            this->m_voiceDc = 0x800 * 0xff;
        }
        else
        {
            // No DC offsets in the MOS8580.
            this->m_waveZero = 0x800;
            this->m_voiceDc = 0;
        }
    }
    ...
    // Set sync source.
    // ----------------------------------------------------------------------------
    void Voice::setSyncSource(Voice* source)
    {
        this->m_wave.setSyncSource(&source->m_wave);
    }

    // ----------------------------------------------------------------------------
    // Register functions.
    // ----------------------------------------------------------------------------
    void Voice::writeControlReg(Reg8 control)
    {
        this->m_wave.writeControlReg(control);
        this->m_envelope.writeControlReg(control);
    }
