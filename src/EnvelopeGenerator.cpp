...
namespace synthaxes::hw::engine::sid
{
    ...
    void EnvelopeGenerator::reset()
    {
        ...
        this->m_state = kRelease;
        this->m_ratePeriod = EnvelopeGenerator::rateCounterPeriod[this->m_release];
    ...
    // periods.
    //
    Reg16 EnvelopeGenerator::rateCounterPeriod[] = {
    ...
    //
    Reg8 EnvelopeGenerator::sustainLevel[] = {
    ...
    // Register functions.
    // ----------------------------------------------------------------------------
    void EnvelopeGenerator::writeControlReg(Reg8 control)
    {
        Reg8 gateNext = control & 0x01;

        // The rate counter is never reset, thus there will be a delay before the
        // envelope counter starts counting up (attack) or down (release).

        // Gate bit on: Start attack, decay, sustain.
        if(!this->m_gate && gateNext)
        {
            this->m_state = kAttack;
            this->m_ratePeriod = EnvelopeGenerator::rateCounterPeriod[this->m_attack];

            // Switching to attack state unlocks the zero freeze.
            this->m_holdZero = false;
        }
        // Gate bit off: Start release.
        else if(this->m_gate && !gateNext)
        {
            this->m_state = kRelease;
            this->m_ratePeriod = EnvelopeGenerator::rateCounterPeriod[this->m_release];
        }

        this->m_gate = gateNext;
    }

    void EnvelopeGenerator::writeAttackDecay(Reg8 attackDecay)
    {
        this->m_attack = (attackDecay >> 4) & 0x0f;
        this->m_decay = attackDecay & 0x0f;
        if(this->m_state == kAttack)
        {
            this->m_ratePeriod = EnvelopeGenerator::rateCounterPeriod[this->m_attack];
        }
        else if(this->m_state == kDecaySustain)
        {
            this->m_ratePeriod = EnvelopeGenerator::rateCounterPeriod[this->m_decay];
        }
    }

    void EnvelopeGenerator::writeSustainRelease(Reg8 sustainRelease)
    {
        this->m_sustain = (sustainRelease >> 4) & 0x0f;
        this->m_release = sustainRelease & 0x0f;
        if(this->m_state == kRelease)
        {
            this->m_ratePeriod = EnvelopeGenerator::rateCounterPeriod[this->m_release];
        }
    }
